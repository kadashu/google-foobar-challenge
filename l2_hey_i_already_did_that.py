...
def find_loop(n, b, maxs):
  x_str = ''.join(sorted(n, reverse=True))
  for idx, max in enumerate(maxs):
    if max == x_str:
      print 'Loop Here! n: ',n,', x: ',x_str,' max:', max,':idx',idx
      return len(maxs)-idx
  maxs.append( x_str )
  x = int(x_str, b)
  y_str = ''.join(sorted(n))
  ...
  if len(z) < k:
    # str
    z = '0' * (k - len(z)) + z
  print 'n: ',n,', x: ',x_str,' - y:',y_str,'= z:',z
  try:
    return find_loop(z, b, maxs)
  except Exception as excep:
    print 'Error! n: ',n,', x: ',x_str,' - y:',y_str,'= z:',z
    print 'Exception type:',type(excep),', msg:', excep.args

def answer(n, b):
  maxs = [ n ]
  return find_loop(n, b, maxs)
